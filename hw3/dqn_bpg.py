...
def learn(env,
    ...
    bpg_logprob = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=act_t_ph,logits=policy_weight)
    ...
    train_fn_policy = minimize_and_clip(optimizer_policy, policy_loss,
                 var_list = policy_vars, clip_val=grad_norm_clipping)
    ...
    for t in itertools.count():
        ...
        # put last_obs into replay_buffer
        idx = replay_buffer.store_frame(last_obs)
        current_four_frames = replay_buffer.encode_recent_observation()
        ...
        #     #take random action
            ...
            action_prob = session.run([prob], feed_dict={obs_t_ph: [current_four_frames]})
            action = np.random.choice(num_actions, 1,action_prob)

        ## e-greedy
        # random_prob = exploration.value(t)
        # random_num = np.random.uniform(0, 1)
        # if not model_initialized or random_num < random_prob:
        #     #take random action
        #     #take random action
        #     action = np.random.randint(0,num_actions)
        # else:
        #     #take max action
        #
        #     actions = session.run([q_s], feed_dict={obs_t_ph: [current_four_frames]})
        ...
        # put current_obs , action, reward, next_obs into replay history

        replay_buffer.store_effect(idx,action,reward,done)
        last_obs = new_obs.copy()
        ...
        if (t > learning_starts and
            ...
            ##### update policy
            policy_error, _ = session.run([policy_loss, train_fn_policy], feed_dict={obs_t_ph:obs_batch,
                                                                    act_t_ph:act_batch,
                                                                    learning_rate_policy:1e-2})

            error, _ = session.run([total_error, train_fn], feed_dict={obs_t_ph:obs_batch,
                                                                    act_t_ph:act_batch,
                                                                    rew_t_ph:rew_batch,
                                                                    obs_tp1_ph:obs_tp1_batch,
