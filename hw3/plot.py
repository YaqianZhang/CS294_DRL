...
def plot_data(data, value="AverageReturn"):
    ...
    for value_i in value:
        sns.tsplot(data=data, time="Timestep", value=value_i, unit="Unit", condition="Condition")
    f, ax = plt.subplots(1, 1)
    for value_i in value:
        ax.plot(data['Timestep'],data[value_i])
...
def get_datasets(fpath, condition=None):
    ...
    for root, dir, files in os.walk(fpath):
        if 'log.txt' in files:
            ...
            ######### no paras stored in the new version-zyq
            exp_name = "test"
